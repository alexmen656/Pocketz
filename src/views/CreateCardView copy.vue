...
onUnmounted(() => {
    if (activeCodeReader) {
        try {
            ...
            console.warn('Error resetting reader on unmount:', e)
        }
        activeCodeReader = null
    }

    const container = document.getElementById('zxing-container')
    if (container) {
...
async function startAdvancedScanning() {
    try {
        ...
        const oldContainer = document.getElementById('zxing-container')
        if (oldContainer && document.body.contains(oldContainer)) {
            const oldVideo = oldContainer.querySelector('video') as HTMLVideoElement | null
            if (oldVideo) {
                oldVideo.pause()
        ...
        const oldVideo = document.getElementById('zxing-video') as HTMLVideoElement | null
        if (oldVideo) {
            oldVideo.pause()
            const stream = oldVideo.srcObject as MediaStream
            if (stream) {
        ...
        `
        closeButton.onclick = () => {
            if (activeCodeReader === codeReader) {
                codeReader.reset()
                activeCodeReader = null
            }
        ...
        const scanTimeout = setTimeout(async () => {
            if (!scanSuccess) {
                ...
                isScanning.value = false
                alert('Barcode scanning timed out. Please try again or enter manually.')
            }
        }, 30000)
        ...
        try {
            ...
            if (result) {
                ...
                if (scannedValue) {
                    ...
                    const detectedFormat = detectBarcodeFormat(scannedValue)
        ...
        activeCodeReader = null
        isScanning.value = false
        alert('Camera access denied or scanning failed. Please enter the barcode manually.')
    }
}
